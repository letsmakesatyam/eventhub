  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 animate-fade-in">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 dark:text-white mb-1">Events</h1>
          <p className="text-gray-500 dark:text-slate-400">Create and manage your events</p>
        </div>
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="bg-gray-50 dark:bg-slate-800 border-b border-gray-100 dark:border-slate-700">
                  <th className="text-left px-6 py-4 font-semibold text-gray-600 dark:text-slate-400">Event</th>
                  <th className="text-left px-4 py-4 font-semibold text-gray-600 dark:text-slate-400">Date</th>
                  <th className="text-left px-4 py-4 font-semibold text-gray-600 dark:text-slate-400">Registrations</th>
                  <th className="text-left px-4 py-4 font-semibold text-gray-600 dark:text-slate-400">Price</th>
                  <th className="text-left px-4 py-4 font-semibold text-gray-600 dark:text-slate-400">Status</th>
                ...
                {events.map(event => (
                  <tr key={event.id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/50 transition-colors">
                    <td className="px-6 py-4">
                      <div className="font-medium text-gray-900 dark:text-white truncate max-w-xs">{event.title}</div>
                      <div className="text-xs text-gray-400 dark:text-slate-500">{event.location}</div>
                    </td>
                    ...
                    <td className="px-4 py-4 font-medium text-gray-900 dark:text-white">
                      {event.price === 0 ? <span className="text-emerald-600 dark:text-emerald-400">Free</span> : `₹${event.price}`}
                    </td>
                    <td className="px-4 py-4">
                      <span className={`badge font-medium ${
                        ...
                        event.status === 'draft' ? 'bg-gray-100 text-gray-600 dark:bg-slate-700 dark:text-slate-400' :
                        event.status === 'cancelled' ? 'bg-red-100 text-red-600 dark:bg-red-900/30 dark:text-red-400' :
                    ...
                    <td className="px-4 py-4">
                      <div className="flex items-center gap-2 justify-end">
                        <button onClick={() => openEdit(event)} className="p-2 text-gray-400 hover:text-violet-600 hover:bg-violet-50 dark:hover:bg-violet-900/20 rounded-lg transition-colors">
                          <Edit2 className="w-4 h-4" />
                        </button>
                        <button onClick={() => handleDelete(event.id)} className="p-2 text-gray-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors">
      ...
      {showModal && (
        <div className="fixed inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-gray-100 dark:border-slate-800 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            <div className="p-6 border-b border-gray-100 dark:border-slate-800 flex items-center justify-between">
              <h2 className="text-xl font-bold text-gray-900 dark:text-white">{editing ? 'Edit Event' : 'Create New Event'}</h2>
              <button onClick={() => setShowModal(false)} className="text-gray-400 hover:text-gray-600 dark:text-slate-500 dark:hover:text-slate-300 text-xl w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-100 dark:hover:bg-slate-800">✕</button>
            </div>
            <form onSubmit={handleSave} className="p-6 space-y-4">
              <div className="grid sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Start Date & Time *</label>
                  <input type="datetime-local" value={form.event_date} onChange={upd('event_date')} className="input-field" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">End Date & Time</label>
                  <input type="datetime-local" value={form.end_date} onChange={upd('end_date')} className="input-field" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">City / Location *</label>
                  <input value={form.location} onChange={upd('location')} className="input-field" placeholder="Mumbai" />
                </div>
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Venue</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Price (₹)</label>
                ...
                <div className="sm:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-1.5">Image URL</label>
