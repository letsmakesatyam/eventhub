...
router.post('/create-order', authenticate, async (req, res) => {
  try {
    ...
    // Store pending registration / reuse existing non-confirmed registration row
    let registration;
...
// POST /api/payments/cancel-registration - Cancel or fail a pending registration
router.post('/cancel-registration', authenticate, async (req, res) => {
  try {
    const { registration_id, status = 'cancelled' } = req.body;
    if (!registration_id) {
      return res.status(400).json({ error: 'Registration ID is required' });
    }

    const normalizedStatus = ['cancelled', 'failed'].includes(status) ? status : 'cancelled';
    ...
    res.json({ message: 'Registration status updated', status: normalizedStatus });
  } catch (err) {
